...
/**
 * Replaces some strings in the array with other strings
 * @param words an array of strings
 * @param reps an association list containing pairs of words (key) and their substitutes (value)
 * @modifies words
...
export const splitWords = (str: string): string[] => {
  ...
  while (j !== str.length) {
    if(str.charAt(j) === ' ') {
      splits.push(j+1);
    } else if(isPunct(str.charAt(j))) {
      splits.push(j+1);
    } else {
      if(j !== 0 && (str.charAt(j-1) !== ' ' && !isPunct(str.charAt(j-1)))) {
