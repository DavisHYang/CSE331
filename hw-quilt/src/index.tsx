...
const getQuilt = (pattern: string, rows: bigint, c?: Color): Quilt => {
  switch (pattern) {
    case "A": return PatternA(rows, c);
    case "B": return PatternB(rows, c);
    case "C": return PatternC(rows, c);
    case "D": return PatternD(rows, c);
    case "E": return PatternE(rows, c);
...
const hasSymmetrize: boolean = params.has("symmetrize");
...
if (pattern === undefined) {
  ...
  try {
    ...
    const result = hasSymmetrize ? symmetrize(getQuilt(pattern, rows, color)): getQuilt(pattern, rows, color);
