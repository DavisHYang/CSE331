...
import { Highlight, parseText } from './parser';
...
const color_list: ColorList = makeColorTree();
...
try {
  ...
  if (word !== null) {
    root.render(<React.StrictMode><ShowColorSet text={word} colors={color_list}/></React.StrictMode>);

  // If the query included a list of lines, then show them highlighted.
  } else if (lines) {
    const highlights: List<Highlight> = parseText(lines);
    root.render(
        <React.StrictMode>
          <ShowHighlights highlights={highlights} colors={color_list}/>
        </React.StrictMode>);
